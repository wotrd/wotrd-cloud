...
public class CaffeineService {
     ...
     * 手动创建缓存
     */
    private static final Cache<Long, Item> cache = Caffeine.newBuilder()
     ...
     */
    LoadingCache<Long, Item> syncCache = Caffeine.newBuilder()
            .maximumSize(100)
            .expireAfterWrite(1, TimeUnit.MINUTES)
            .build(k -> new Item());
    /**
    ...
    public String getById(Long id) {
        ...
        log.info("get id:{}, result:{}", id, item);
        ...
        Item item1 = cache.get(id, k -> new Item());
        //异步查询
        asyncCache.get(id).thenAccept(item2 -> {
            log.info(JSON.toJSONString(item2));
        });
    ...
    public String update(Item item) {
        ConcurrentMap<Long, Item> stringItemConcurrentMap = cache.asMap();
        cache.cleanUp();
